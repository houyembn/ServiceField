...
function ServiceCaseDetails() {
    ...
    const [updatedServiceCase, setUpdatedServiceCase] = useState(null);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setUpdatedServiceCase(serviceCaseResponse.data);
            } catch (error) {
    ...
    const handleInputChange = (e) => {
        const { name, value } = e.target;
        setUpdatedServiceCase(prevCase => ({ ...prevCase, [name]: value }));
    };

    const handleSaveClick = async () => {
        try {
            await axios.put(`https://localhost:7141/ServiceField/Case/${id}`, updatedServiceCase);
            setEditMode(false);
            setShowSuccessModal(true);
            setTimeout(() => {
                setShowSuccessModal(false);
    ...
    return (
        <div className="flex">
                    ...
                    <Container fluid>
                        <Row className="justify-content">
                            <Col md={8}>
                                ...
                                <Card style={{ width: '100%', boxShadow: '0 4px 8px rgba(0,0,0,0.1)', padding: '20px' }}>
                                    <Card.Body>
                                        <Card.Title className="title">Service Case Information:</Card.Title><br />

                                        <Row className="mb-3">
                                            <Card.Text as={Col}>
                                                <span className="custom-label">Service Case Number:</span>
                                                {editMode ? (
                                                    <input
                                                        type="text"
                                                        name="productSerialNumber"
                                                        value={updatedServiceCase.productSerialNumber}
                                                        onChange={handleInputChange}
                                                        className="form-control"
                                                    />
                                                ) : (
                                                    <span>{serviceCase.productSerialNumber}</span>
                                                )}
                                            </Card.Text>
                                        </Row>

                                        {/* Add other fields here in a similar way */}
                                        {/* For example, affected company */}
                                        <Row className="mb-3">
                                            <Card.Text as={Col}>
                                                <span className="custom-label">Affected Company:</span>
                                                {editMode ? (
                                                    <Form.Select
                                                        name="affectedCompany"
                                                        value={updatedServiceCase.affectedCompany}
                                                        onChange={handleInputChange}
                                                    ...
                                                    <span>{serviceCase.affectedCompany}</span>
                                                )}
                                            </Card.Text>
                                        </Row>
                                ...
                                <div className="mt-4">
                                    {editMode ? (
                                        <>
                                            <Button onClick={handleSaveClick} className="updateBtn">
                                                Save
                                            </Button>
            ...
            <Modal show={showSuccessModal} onHide={() => setShowSuccessModal(false)}>
                ...
                <Modal.Body>Service case updated successfully.</Modal.Body>
                <Modal.Footer>
            ...
            <Modal show={showFailModal} onHide={() => setShowFailModal(false)}>
                ...
                <Modal.Body>Failed to update the service case.</Modal.Body>
                <Modal.Footer>
