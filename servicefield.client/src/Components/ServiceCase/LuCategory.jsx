...
function LuCategory() {
    ...
    useEffect(() => {
        const fetchSkills = async () => {
            try {
                const response = await axios.get('https://localhost:7141/ServiceField/Category');
    ...
    const handleModalSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post('https://localhost:7141/ServiceField/Category', { type: newSkill });
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`https://localhost:7141/ServiceField/Category/${id}`);
                                ...
                                <Card key={skill.id} className="mb-3" >
                                    <Card.Body >
                                        <div className="d-flex justify-content-between align-items-center">
                                            <div>
                                                <label className="custom-label">Category type :</label>
                    ...
                    <Form onSubmit={handleModalSubmit}>
                        <Form.Group controlId="formNewSkill">
                            <Form.Control
                                ...
                                type="text"
                                placeholder="Put a new category"
                                value={newSkill}
